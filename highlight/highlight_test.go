...
function Probe(@specialize config: Config, batch: Axis) -> tensor.Pair {
  ...
  row
	...
	assertClassifiesAt(t, result, splice+1, "batch", chroma.NameProperty, "tree-sitter.variable.other.member")
	assertClassifiesAt(t, result, strings.Index(source, "$config.row"), "$", chroma.LiteralStringInterpol, "tree-sitter.punctuation.special")
	assertClassifiesAt(t, result, strings.Index(source, "$config.row")+len("$config."), "row", chroma.NameProperty, "tree-sitter.variable.other.member")
